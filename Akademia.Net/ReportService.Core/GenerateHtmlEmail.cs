...
namespace ReportService.Core
{
    public class GenerateHtmlEmail
    {
        public string GenerateErrors(List<Error> errors, int interval)
        {
            ...
            html += @"</table> <br /><br /> <i>Autmatyczne wiadomość wysłana z aplikacji ReportService.</i>";
        ...
        public string GenerateReport(Report report)
        {
            ...
            var html = $@"Raport {report.Title} z dnia 
                        {report.Date.ToString("dd-MM-yyyy")}. <br /> <br />";

            if (report.Position != null && report.Position.Any())
            {
                html += @"
                        <table border=1 cellpadding=5 cellspacing=1>
                            <tr>
                ...
                foreach (var position in report.Position)
                {
                    html +=
                        $@"<tr>
            ...
            html += @"<br /> <br /><i>Autmatyczne wiadomość wysłana z aplikacji ReportService.</i>";
